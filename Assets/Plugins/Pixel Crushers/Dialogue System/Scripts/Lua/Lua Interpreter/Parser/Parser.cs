...
namespace Language.Lua
{
    public partial class Parser
    {
        ...
        private Chunk ParseChunk(out bool success)
        {
            ...
            while (true)
            {
                while (true)
                {
                    ...
                    int safeguard = 0;
                    while (semiSuccess && safeguard < 10)
                    {
                        MatchTerminal(';', out semiSuccess);
                        if (semiSuccess)
                        {
                            var spaceSuccess = true;
                            int safeguard2 = 0;
                            while (spaceSuccess && safeguard2 < 10)
                            {
                                MatchTerminal(' ', out spaceSuccess);
                            }
                        }
                        safeguard++;
                    }
        ...
        private IfStmt ParseIfStmt(out bool success)
        {
            ...
            MatchTerminalString(";", out optionalSuccess);
        ...
        private string ParseFloatNumber(out bool success)
        {
            ...
            while (true)
            {
                ...
                //[PixelCrushers] Handle positive and negative exponents:
                ch = MatchTerminal('-', out success);
                if (!success) ch = MatchTerminal('+', out success);
                if (success) { text.Append(ch); }
                success = true;
