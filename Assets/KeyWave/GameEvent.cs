...
public static class GameEvent
{
    ...
    public static event Action<YarnEvents.YarnEvent> OnYarnEventActive;

    public static void StartYarnEvent(YarnEvents.YarnEvent yarnEvent) => OnYarnEventActive?.Invoke(yarnEvent);


    public static event Action<YarnEvents.YarnEvent> OnYarnEventEnd;
    public static void EndYarnEvent(YarnEvents.YarnEvent yarnEvent) => OnYarnEventEnd?.Invoke(yarnEvent);

    public static event Action<string> OnYarnSignal;
    public static void YarnSignal(string signal) => OnYarnSignal?.Invoke(signal);

    public static event Action<string> OnYarnNodeEnd;
    public static void YarnNodeEnd(string node) => OnYarnNodeEnd?.Invoke(node);

    public static event Action<string> OnYarnNodeStart;
    public static void YarnNodeStart(string node) => OnYarnNodeStart?.Invoke(node);



    //RELOCATE
    public delegate void RelocateDelegate(Actor actor, Transform room);
    public static event RelocateDelegate OnActorExitRoom;
    public static event RelocateDelegate OnActorEnterRoom;

    public static event Action<string> OnPlayerEnterLocation;


    public static void MoveActorToRoom(Actor actor, Transform destination)
    {
        actor.location = destination;
        OnActorEnterRoom?.Invoke(actor, destination);
    }
    public static void RemoveActorFromRoom(Actor actor)
    {
        OnActorExitRoom?.Invoke(actor, actor.location);
    }

    public static void MovePlayerToRoom(string destination) => OnPlayerEnterLocation?.Invoke(destination);
