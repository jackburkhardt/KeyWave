...
using CurveRenderer = UnityEditor.Enemeteen.CurveRenderer;
using IntCurveRenderer = UnityEditor.Enemeteen.IntCurveRenderer;
using BoolCurveRenderer = UnityEditor.Enemeteen.BoolCurveRenderer;
using NormalCurveRenderer = UnityEditor.Enemeteen.NormalCurveRenderer;
using CurveWrapper = UnityEditor.Enemeteen.CurveWrapper;
using CurveUtility = UnityEditor.Enemeteen.CurveUtility;
using CurveSelection = UnityEditor.Enemeteen.CurveSelection;
using CurveEditor = UnityEditor.Enemeteen.CurveEditor;
...
namespace UnityEditorInternal.Enemeteen {
	static internal class AnimationWindowUtility {
		...
		public static void CreateDefaultCurves(AnimationWindowState state, AnimationClip animationClip, EditorCurveBinding[] properties) {
			properties = UnityEditor.Enemeteen.RotationCurveInterpolation.ConvertRotationPropertiesToDefaultInterpolation(animationClip, properties);
			...
			if (curve.isPPtrCurve) {
				...
				if (type == typeof(bool)) {
					...
					if (animationCurve.length <= 1) {
						...
						if (keyIndex != -1) {
							UnityEditor.Enemeteen.CurveUtility.SetKeyModeFromContext(animationCurve, keyIndex);
		...
		public static EditorCurveBinding[] PropertyModificationsToEditorCurveBindings(PropertyModification[] modifications, GameObject rootGameObject, AnimationClip animationClip) {
			...
			for (int i = 0; i < modifications.Length; ++i) {
				...
				if (AnimationUtility.PropertyModificationToEditorCurveBinding(modifications[i], rootGameObject, out binding) != null) {
					EditorCurveBinding[] additionalBindings = UnityEditor.Enemeteen.RotationCurveInterpolation.RemapAnimationBindingForAddKey(binding, animationClip);
		...
		public static void SyncTimeArea(UnityEditor.Enemeteen.TimeArea from, UnityEditor.Enemeteen.TimeArea to) {
		...
		public static void DrawOutOfRangeOverlay(Rect rect, Color color, float startOfClipPixel, float endOfClipPixel) {
			...
			if (startOfClipPixel > rect.xMin) {
				...
				UnityEditor.Enemeteen.TimeArea.DrawVerticalLine(startRect.xMax, startRect.yMin, startRect.yMax, lineColor);
			}
			...
			UnityEditor.Enemeteen.TimeArea.DrawVerticalLine(endRect.xMin, endRect.yMin, endRect.yMax, lineColor);
		}
		...
		public static void ControllerChanged() {
			foreach (EnemeteenWindow animationWindow in EnemeteenWindow.GetAllAnimationWindows())
