...
using UnityEditor;
using ControlPointRenderer = UnityEditor.Enemeteen.ControlPointRenderer;
using DopeSheetEditorRectangleTool = UnityEditor.Enemeteen.DopeSheetEditorRectangleTool;
using AnimationWindowStyles = UnityEditor.Enemeteen.AnimationWindowStyles;
using ChangedCurve = UnityEditor.Enemeteen.ChangedCurve;
using CurveMenuManager = UnityEditor.Enemeteen.CurveMenuManager;
using KeyIdentifier = UnityEditor.Enemeteen.KeyIdentifier;
using TimeArea = UnityEditor.Enemeteen.TimeArea;
using CurveUpdater = UnityEditor.Enemeteen.CurveUpdater;
		...
		class DopeSheetControlPointRenderer {
			...
			public void AddUnselectedKey(DrawElement element) {
				...
				if (element.texture != null) {
					...
					rect.size = new Vector2(m_DefaultDopeKeyIcon.width/2f, m_DefaultDopeKeyIcon.height/2f);
