...
namespace PerilsAndPitfalls
{
    public class OptionsListView : DialogueViewBase
    {
        ...
        {
            //set option list view to bottom of scroll rect
           ...
           // StartCoroutine(Effects.FadeAlpha(canvasGroup, 0, 1, fadeTime));
            ...
            OptionView CreateNewOptionView()
            {
                ...
                optionView.transform.SetParent(panel.transform, false);
            ...
            StartCoroutine(Effects.AutoScroll(scrollRect, 5f));
            ...
            void OptionViewWasSelected(DialogueOption option)
            {
                ...
                {
                    // make each current option view uninteractable after selecting an option
                    foreach (OptionView _option in optionViews) _option.interactable = false;
                    // reset cached list so that new option views can spawn
                    optionViews.Clear();
                    // create divider
                    Instantiate(divider, panel.transform);
