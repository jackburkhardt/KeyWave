...
namespace PerilsAndPitfalls
{
    ...
    public static class Effects
    {
        ...
        /// <param name="stopToken">A <see cref="CoroutineInterruptToken"/> that
        /// can be used to interrupt the coroutine.</param>
        public static IEnumerator Typewriter(TextMeshProUGUI text, float lettersPerSecond, Action onCharacterTyped, CoroutineInterruptToken stopToken = null, int startIndex = 0)
        {
            ...
            // Start with everything invisible, except character name
        ...
        //custom goobness
        public static IEnumerator AutoScroll(ScrollRect scrollRect, float unitsPerSecond)
        {
            float initialVerticalPosition = scrollRect.verticalNormalizedPosition;
            float animationProgress = 0;

            // we move the scroll rect to the lowest vertical position
            while (scrollRect.verticalNormalizedPosition > 0 && animationProgress < 1)
            {
                animationProgress += unitsPerSecond * Time.deltaTime;

                scrollRect.verticalNormalizedPosition = Mathf.SmoothStep(initialVerticalPosition, 0, animationProgress);
                yield return null;
            }
        }
    ...
    public class LineView : DialogueViewBase
    {
        ...
        [SerializeField]
        int _dialogueNamePadding;
        string _leftHandSideText;
        [SerializeField]
        private GameObject _audioMachine;
        ...
        private void Awake()
        {
            canvasGroup.alpha = 1;
            canvasGroup.blocksRaycasts = false;
            foreach (RectTransform panelItem in _textContainer.GetComponentsInChildren<RectTransform>()) panelItem.gameObject.SetActive(false);
        ...
        private IEnumerator DismissLineInternal(Action onDismissalComplete)
        {
           ...
           // canvasGroup.blocksRaycasts = true;
            ...
            // let's spawn a divider here to indicate that the line has been fully dismissed.
            Instantiate(_dividerPrefab, _textContainer);
        ...
        public override void InterruptLine(LocalizedLine dialogueLine, Action onInterruptLineFinished)
        {
            ...
            lineText.text = ParseTextMarkup(dialogueLine);
        ...
        string ParseTextMarkup(LocalizedLine dialogueLine)
        {

            string text = dialogueLine.Text.Text;
          
            int[] markupIndeces = new int[dialogueLine.Text.Attributes.Count * 2];
            ...
            foreach (var attribute in dialogueLine.Text.Attributes)
            {
                ...
                //populate array with all indeces that will be used
                markupIndeces[arrayIndex] = attribute.Position;
                markupIndeces[arrayIndex + 1] = attribute.Position + attribute.Length;
                arrayIndex += 2;

                switch (attribute.Name)
                {
                    case "i":
                        hashTable.Add(attribute.Position, "<i>");
                        hashTable.Add(attribute.Position + attribute.Length, "</i>");
                        break;
                    case "negative":
                        hashTable.Add(attribute.Position, "<style=\"Negative\">");
                        hashTable.Add(attribute.Position + attribute.Length, "</style>");
                        break;
                    case "positive":
                        hashTable.Add(attribute.Position, "<style=\"Positive\">");
                        hashTable.Add(attribute.Position + attribute.Length, "</style>");
                        break;
                    case "emphasis":
                        hashTable.Add(attribute.Position, "<style=\"Emphasis\">");
                        hashTable.Add(attribute.Position + attribute.Length, "</style>");
                        break;
                    case "important":
                        hashTable.Add(attribute.Position, "<style=\"Important\">");
                        hashTable.Add(attribute.Position + attribute.Length, "</style>");
                        break;
                    case "character":
                        //replace colon with empty space
                        string nameSubstring = text.Substring(attribute.Position, attribute.Length);                      
                        text = nameSubstring.Replace(":", " ") + text.Substring(attribute.Position + attribute.Length);

                        hashTable.Add(attribute.Position, $"<style=\"Name\"><style=\"{attribute.Properties["name"]}\">");
                        hashTable.Add(attribute.Position + attribute.Length, "</style></style>");
                        break;
                }

            }

            //reverse array order. markup must be inserted in reverse, as indeces change otherwise
            Array.Sort(markupIndeces, (a, b) => b.CompareTo(a));

            for (int i = 0; i < markupIndeces.Length; i++)
            {
                if (hashTable.ContainsKey(markupIndeces[i]))
                {
                    text = text.Insert(markupIndeces[i], hashTable[markupIndeces[i]]);
                }
            }
        ...
        private IEnumerator RunLineInternal(LocalizedLine dialogueLine, Action onDialogueLineFinished)
        {
           ...
           // lineText = Instantiate(_textPrefab).GetComponent<TextMeshProUGUI>();
           // lineText.transform.SetParent(_textContainer);
            ...
            IEnumerator PresentLine()
            {
                ...
                if (useTypewriterEffect)
                {
                    ...
                    // in on visible text).
                    lineText.maxVisibleCharacters = _leftHandSideText.Length;
                }
                else
                {
                ...
                canvasGroup.alpha = 1f; //needed for autoscroll effect
                Canvas.ForceUpdateCanvases();
                yield return StartCoroutine(Effects.AutoScroll(m_scrollRect, 5f));
        ...
        public override void UserRequestedViewAdvancement()
        {
            ...
            if (currentLine == null || lineText.maxVisibleCharacters <= _leftHandSideText.Length)
            {
                return;
            }
            ...
            // interrupt the line instead.
                ...
                lineText.color -= new Color(0.3f, 0.3f, 0.3f, 0);
